...
func defaultStyles() styles {
	...
	return styles{
		...
		subheader:    lipgloss.NewStyle().Foreground(muted),
		...
		listTitle:    lipgloss.NewStyle().Bold(true).Foreground(textSoft),
		listSelected: lipgloss.NewStyle().Foreground(text).Bold(true),
		listNormal:   lipgloss.NewStyle().Foreground(textSoft),
		listDesc:     lipgloss.NewStyle().Foreground(muted),
...
func (d listDelegate) Render(w io.Writer, m list.Model, index int, item list.Item) {
	...
	nameStyle := d.styles.listNormal.Background(rowBg)
	if isSelected {
		nameStyle = d.styles.listSelected.Background(rowBg)
	}

	markerStyle := lipgloss.NewStyle().Background(rowBg)
	marker := markerStyle.Render("  ")
	if isSelected {
		marker = d.styles.marker.Background(rowBg).Render("› ")
	}
	nameLine := marker + nameStyle.Render(i.label)
	descLine := d.styles.listDesc.Background(rowBg).Render(i.description)
	rowStyle := lipgloss.NewStyle().Width(m.Width()).Background(rowBg)
	_, _ = fmt.Fprintln(w, rowStyle.Render(nameLine))
	if i.description != "" {
		_, _ = fmt.Fprintln(w, rowStyle.Render("  "+descLine))
	}
}
...
func NewWizard(defaultLanguage string, defaultFramework string) tea.Model {
	...
	spin.Style = lipgloss.NewStyle().Foreground(styles.accent)
	options := map[string][]string{}
	libOptions := map[string][]string{}
...
func (m model) renderNameInput() string {
	...
	return lipgloss.JoinVertical(lipgloss.Left, label, "", box, "", help)
}
